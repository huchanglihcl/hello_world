...
- [1流的概念](#1流的概念)
- [2 创建流的5种方法](#2-创建流的5种方法)
  - [2.1 通过Collection系列集合提供的串行流：stream()、并行流：paralleStream()](#2.1-通过Collection系列集合提供的串行流：stream()、并行流：paralleStream())
  - [2.2 通过Arrays中的静态方法stream(T[] array)获取数组流](#2.2-通过Arrays中的静态方法stream(T[]-array)获取数组流)
  - [2.3 通过Stream类中的静态方法of()](#2.3-通过Stream类中的静态方法of())
  - [2.4 使用Stream类的静态方法iterate创建无限流](#2.4-使用Stream类的静态方法iterate创建无限流)
  - [2.5 使用Stream类的静态方法generate创建无限流](#2.5-使用Stream类的静态方法generate创建无限流)
- [3 Stream中间操作](#3-Stream中间操作)
  - [3.1 filter-过滤](#31-filter-过滤)
  - [3.2 limit-限定元素数量](#3.2-limit-限定元素数量)
  - [3.3 skip-跳过元素](#3.3-skip-跳过元素)
  - [3.4 distinct-去重](#3.4-distinct-去重)
  - [3.5  map映射](#3.5--map映射)
  - [3.6flatMap映射](#36flatMap映射)
  - [3.7 自然排序](#3.7-自然排序)
  - [3.8 指定排序](#3.8-指定排序)
- [4 终止Stream操作](#4-终止Stream操作)
  - [4.1 allMatch-检查是否匹配所有元素](#4.1-allMatch-检查是否匹配所有元素)
  - [4.2 anyMatch-检查是否至少匹配一个元素](#4.2-anyMatch-检查是否至少匹配一个元素)
  - [4.3 noneMatch-检查是否所有元素都没有匹配到](#4.3-noneMatch-检查是否所有元素都没有匹配到)
  - [4.4 findFirst-返回第一个元素](#4.4-findFirst-返回第一个元素)
  - [4.5 findAny-返回当前流中的任意一个元素](#4.5-findAny-返回当前流中的任意一个元素)
  - [4.6 count-返回流中元素总个数](#4.6-count-返回流中元素总个数)
  - [4.7 max-返回流中的最大值](#4.7-max-返回流中的最大值)
  - [4.7 min-返回流中的最小值](#4.7-min-返回流中的最小值)
  - [4.8 reduce-将流中元素反复结合起来，得到一个值](#4.8-reduce-将流中元素反复结合起来，得到一个值)
  - [4.9 collect-将流转换为其他形式](#4.9-collect-将流转换为其他形式)
  - [4.10 Collectors类中的常用方法](#4.10-Collectors类中的常用方法)
    - [4.10.1 counting-统计数量](#4.10.1-counting-统计数量)
    - [4.10.2 averagingDouble-求平均值并转换成Double类型](#4.10.2-averagingDouble-求平均值并转换成Double类型)
    - [4.10.3 summingDouble-求和并转换成Double类型](#4.10.3-summingDouble-求和并转换成Double类型)
    - [4.10.4 maxBy-根据函数条件求最大值](#4.10.4-maxBy-根据函数条件求最大值)
    - [4.10.5 groupingBy-分组](#4.10.5-groupingBy-分组)
    - [4.10.6 partitioningBy-分区](#4.10.6-partitioningBy-分区)
    - [4.10.7 summarizingDouble-计算方法总括函数](#4.10.7-summarizingDouble-计算方法总括函数)
    - [4.10.8 joining-连接字符串](#4.10.8-joining-连接字符串)
